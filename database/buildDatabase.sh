...
if [[ $# -eq 0 ]]; then
  DOWNLOAD_DATE=$(wget -q -O- https://dumps.wikimedia.your.org/trwiki/ | grep -Po '\d{8}' | sort | tail -n1)
else
  if [ ${#1} -ne 8 ]; then
...
DOWNLOAD_URL="https://dumps.wikimedia.your.org/trwiki/$DOWNLOAD_DATE"
TORRENT_URL="https://tools.wmflabs.org/dump-torrents/trwiki/$DOWNLOAD_DATE"

SHA1SUM_FILENAME="trwiki-$DOWNLOAD_DATE-sha1sums.txt"
REDIRECTS_FILENAME="trwiki-$DOWNLOAD_DATE-redirect.sql.gz"
PAGES_FILENAME="trwiki-$DOWNLOAD_DATE-page.sql.gz"
LINKS_FILENAME="trwiki-$DOWNLOAD_DATE-pagelinks.sql.gz"
